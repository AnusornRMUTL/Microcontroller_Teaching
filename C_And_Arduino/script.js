...
window.selectAnswer = function(qIndex, selectedOptIndex) {
    ...
    // Scroll slightly down to show the reason clearly if needed
    ...
    if (userScore >= passThreshold) {
        ...
        resDiv.innerHTML = `
            <div style="background: #fef2f2; padding: 30px; border-radius: 12px; border: 2px solid #ef4444; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                ...
                <button class="action-btn secondary-btn" style="margin-top: 20px; font-size: 1.1rem; padding: 12px 24px;" onclick="renderSection('quiz')">ทดสอบใหม่ (สุ่มข้อสอบใหม่)</button>
            </div>
        `;
    }
...
window.generateCertificate = function() {
    ...
    // Borders
    ctx.lineWidth = 15;
    ctx.strokeStyle = '#00979C'; // Arduino Teal
    ...
    ctx.font = '22px "Sarabun", sans-serif';
    ctx.fillStyle = '#1e293b';
    ctx.fillText('ได้ผ่านการทดสอบความรู้ โมดูล 1: ภาษา C และ Arduino', canvas.width/2, 320);
